...
*/
{
    ...
    // by default, non-significant content is whole line
    if (linecopy != NULL) {
        *endptr = *linecopy;
    }
    ptr = line;
    ...
    if (2 != number_of_commas && 3 != number_of_commas) {
        return INVALID_FORMAT;  // must have 2 or 3 commas...
    }
 ...
 *     2 - get_next_assignment : no valid assignment found                                                  *
 ************************************************************************************************************/
enum ASSIGNMENT_ERRORS get_next_assignment(char *filename, unsigned int *exponent, int *bit_min, int *bit_max, LINE_BUFFER *key,
                                           int verbosity)
{
    FILE *f_in;
    ...
    do {
        ...
        if (verbosity >= 1) {
            ...
            case LONG_LINE:
                ...
                break;
            case NO_FACTOR_EQUAL:
                printf("doesn't begin with Factor=\n");
                break;
            case INVALID_FORMAT:
            ...
            case INVALID_DATA:
                ...
                break;
            default:
                printf("unknown error on >%s<", line);
                break;
            }
        }
    ...
    if (addfile == NULL) {
        ...
        if (*addfilesstatus == 0) /* status was 2 before and is 0 now, try to delete addfile! */
        {
            if (remove(addfilename) != 0) {
                ...
                printf("         Disabled worktodo.add feature until mfaktc\n");
                return CANT_RENAME;
            }
        }
    }
